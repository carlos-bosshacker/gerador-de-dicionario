...
def gerar_e_testar_dicionario(modo_ataque, bssid, arquivo_cap, tamanho_maximo):
    try:
        opcoes = ["-a", modo_ataque, "-b", bssid, arquivo_cap]
...
if __name__ == "__main__":
    ...
    arquivo_cap = input("Digite o nome do arquivo de captura (.cap): ")
    tamanho_maximo = int(input("Digite o tamanho máximo para as combinações: "))

    t = threading.Thread(target=gerar_e_testar_dicionario, args=(modo_ataque, bssid, arquivo_cap, tamanho_maximo))
    t.start()
    t.join()
